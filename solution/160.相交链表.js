...
/**
 * 方案1：
 * 穷举，o(n^2)，可以用hash map优化
 * 
 * 方案2：
 * 先得到两个链表的长度，相减得到a。
 * 在长的那一条，从第a+1个开始，两个指针同时前进判断是否相等，直到末尾
 * 时间复杂度为长的那一条的2倍，m > n  O(2m)
 * 
 * 方案3：
 * 试想，链表A B如果不等长，那么互相将对方加在自己的头部，
 * 这样不就等长了？
 * 一旦等长，双指针同时前进，第一次相等的节点就是相交的起始节点了。
 * 
 * 做法：双指针从头开始，若不相等则前进一个节点，
 * 若为null则切换另一个链表，这样就相当于将对方链表拼接在头部了。
 * 接下来如果有有相等且不为null，则为相交。
 * 若为null，则代表两个指针走到头了，不相交。
 * 
